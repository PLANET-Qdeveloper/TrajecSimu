...
def dropPoint2Coordinate(drop_points, coord0, mag_dec=-7.53):
    ...
    mat_rot = np.array([[np.cos(mag_dec_rad), -1 * np.sin(mag_dec_rad)], [np.sin(mag_dec_rad), np.cos(mag_dec_rad)]])
...
def setKmlCircle(kml, point_center, radius, name=None, plot_center=True, infill=False):
    if plot_center:
        point = kml.newpoint(name=name, coords=[tuple(point_center)])
    else:
        point = None

    circleplots = getCirclePlot(center_coord=point_center, radius_meter=radius)
    if infill:
...
def setKmlByDicts(dicts, kml=None):
    ...
    for dict in dicts:
        ...
        if dict["type"] == "point":
            point = dict["coord"][::-1]
            kml.newpoint(name=name, coords=[tuple(point)])
            ...
            point, pol = setKmlCircle(kml, point, radius, name=name, plot_center=is_plot_center, infill=is_infill)
...
def getRailCoordByRegulation(regulations):
    for regulation in regulations:
        if regulation["name"] == "rail":
            return (np.array(regulation["center"]), regulation["radius"])
    return None
