...
# 必須の引数
parser.add_argument("scatter_folder", help="誤差分散による各地上風向風速に対する落下分散が入っているフォルダ名")
parser.add_argument("place", help="射点の地名(izu_sea or noshiro_sea)")
parser.add_argument("output_dir", help="出力先ディレクトリ(拡張子は省略してください)")

parser.add_argument(
    "-f",
    "--forecast_point_folder",
    help="誤差分散に用いた予報風通りに風が吹いた場合の落下分散が入っているフォルダ(プロットする場合のみ指定)",
)
parser.add_argument("-s", "--shift_to_point", help="シフト先予報ファイル")
...
wind_direction_array = np.arange(0.0, 360.0, 22.5)
wind_speed_array = np.arange(1.0, 8.0)
