...
class PostProcess_single:
    ...
    def postprocess(self, process_type="all"):
        ...
        if process_type == "location":
            ...
            time = time[
                time <= self.myrocket.trajectory.landing_time
            ]  # array before landing: cut off useless after-landing part
    ...
    def launch_clear_v(self, time, flag_tipoff=False):
        ...
        if flag_tipoff:
            ...
            print(
                " 1st lug clear at t=",
                "{0:.5f}".format(time_1stlug_off),
                "[s], airspeed=",
                "{0:.3f}".format(air_speed_1stlug_clear),
                "[m/s], AoA=",
                "{0:.3f}".format(np.rad2deg(AoA_1stlug_clear)),
                " [deg]",
            )
            print(
                " 2nd lug clear at t=",
                "{0:.5f}".format(time_2ndlug_off),
                "[s], airspeed=",
                "{0:.3f}".format(air_speed_2ndlug_clear),
                "[m/s], AoA=",
                "{0:.3f}".format(np.rad2deg(AoA_2ndlug_clear)),
                " [deg]",
            )
            print(
                " nozzle  clear at t=",
                "{0:.5f}".format(time_nozzle_off),
                "[s], airspeed=",
                "{0:.3f}".format(air_speed_nozzle_clear),
                "[m/s], AoA=",
                "{0:.3f}".format(np.rad2deg(AoA_nozzle_clear)),
                " [deg]",
            )
    ...
    def echo_thrust(self, show=False):
        ...
        if show:
            ...
            try:
                if self.myrocket.Params.curve_fitting:
                    time_for_fit = np.linspace(
                        self.myrocket.Params.time_array[0], self.myrocket.Params.time_array[-1], 1e4
                    )
                    thrust_fit = self.myrocket.Params.thrust_function(time_for_fit)
                    thrust_fit[thrust_fit < 0.0] = 0.0
        ...
        if self.myrocket.Params.It_poly_error > 5.0:
            raise CurveFittingError(
                "!!!  WARNING: POOR THRUST CURVE FITTING. Recommend: curve_fitting=True in config file. !!!"
            )
    ...
    def dropPoint2Coordinate(drop_points, coord0, mag_dec=-7.53):
        ...
        mat_rot = np.array(
            [[np.cos(mag_dec_rad), -1 * np.sin(mag_dec_rad)], [np.sin(mag_dec_rad), np.cos(mag_dec_rad)]]
        )
    ...
    def get_flight_detail(self, time, show=False):
        ...
        for i in range(n):
            T[i], p[i], rho[i], a[i] = self.myrocket.trajectory.standard_air(
                self.myrocket.trajectory.solution[i, 2]
            )  # provide altitude=u[2]
        # END IF
        ...
        # index of max Q
        Q_max = np.argmax(0.5 * rho * speed**2.0)
        # index of max. speed
        ...
        wind_vec = self.myrocket.trajectory.Params.wind(
            self.myrocket.trajectory.solution[Q_max, 2] * 4.0
        )  # provide altitude=u[2]
        ...
        M = speed / a
        Q = 0.5 * rho * speed**2.0
        AoA = np.arctan(wind_speed / speed) * 180.0 / np.pi
        ...
        df_acc.to_csv("./data/results/acc.csv", sep="\t")
        summ = {
            "time": t,
            "AoA": AoA,
            "Q": Q,
        ...
        df_quat.to_csv("./data/results/quaternion.csv", index=False)
        ...
        df_coordinates.to_csv("./data/results/coordinates.csv", index=False)

        # wada write
        def read_df_and_convert_to_kml(initial_latitude, initial_longitude, initial_ground_elevation, output_kml_file):
            # KML オブジェクトを作成
            kml = simplekml.Kml()

            # 線を作成して KML に追加
            line = kml.newlinestring(name="Rocket Trajectory", extrude=1, tessellate=0)
            ...
            for index, row in df_coordinates.iterrows():
                ...
                # 相対座標を計算
                relative_latitude, relative_longitude = calculate_relative_coordinates(
                    initial_latitude, initial_longitude, x, y
                )
                relative_altitude = initial_ground_elevation + z

                # 線を構成する座標を追加
                coords.append((relative_longitude, relative_latitude, relative_altitude))
            ...
            # 色を設定
            line.style.linestyle.color = simplekml.Color.red  # 例: 青色

            # KML ファイルに保存
            kml_file_path = os.path.join(output_folder, output_kml_file)
            kml.save(kml_file_path)
            print(f"KML ファイル '{kml_file_path}' が正常に保存されました。")

        def calculate_relative_coordinates(initial_latitude, initial_longitude, dx, dy):
            # 緯度経度の相対座標を計算
            earth_radius = 6371000  # 地球の半径（メートル）
            lat1, lon1 = radians(initial_latitude), radians(initial_longitude)
            dN, dE = dy, dx

            lat2 = lat1 + dN / earth_radius
            lon2 = lon1 + dE / (earth_radius * cos(lat1))

            return degrees(lat2), degrees(lon2)

        # 初期座標と地面からの高度
        initial_latitude = self.myrocket.Params.loc_lat
        initial_longitude = self.myrocket.Params.loc_lon
        initial_ground_elevation = self.myrocket.Params.loc_alt

        # 保存先のフォルダとファイル名を指定
        output_folder = "./data/results"
        output_kml_file = "output.kml"

        # CSV ファイルから読み込んだデータを KML に変換
        read_df_and_convert_to_kml(initial_latitude, initial_longitude, initial_ground_elevation, "output.kml")
        ...
        # ------------------------------
        maxq_dict = {
            "max_Q": 0.5 * rho[Q_max] * speed[Q_max] ** 2.0,  # max. dynamic pressure
            "time": time[Q_max],  # time
            "free_stream_pressure": p[Q_max],
            "free_stream_temperature": T[Q_max],
            "free_stream_Mach: ": speed[Q_max] / a[Q_max],
            "wind_speed: ": wind_speed,
            "AoA_for_gustrate2": np.arctan(wind_speed / speed[Q_max]) * 180.0 / np.pi,
        }
        tmp_dict = {
            "max_Mach": np.array([speed[M_max] / a[M_max], time[M_max]]),  # [max. Maxh number, time]
        ...
        if show:
            ...
            print(
                " Max. Mach number: ",
                "{0:.3f}".format(speed[M_max] / a[M_max]),
                " at t=",
                "{0:.2f}".format(time[M_max]),
                "[s]",
            )
            print(
                " Max. Q: ",
                "{0:6.2e}".format(0.5 * rho[Q_max] * speed[Q_max] ** 2.0),
                "[Pa] at t=",
                "{0:.2f}".format(time[Q_max]),
                "[s]",
            )
            print(" Max. speed: ", "{0:.1f}".format(speed[v_max]), "[m/s] at t=", "{0:.2f}".format(time[v_max]), "[s]")
            print(
                " MAX acceleration: ",
                "{0:.3f}".format(self.myrocket.trajectory.max_accel),
                " at t=",
                "{0:.4f}".format(int(self.myrocket.trajectory.max_accel)),
                "[s]",
            )
            print(
                " Max. altitude: ",
                "{0:.1f}".format(self.myrocket.trajectory.solution[h_max, 2]),
                "[m] at t=",
                "{0:.2f}".format(time[h_max]),
                "[s]",
            )
            print(" total flight time: ", "{0:.2f}".format(self.myrocket.trajectory.landing_time), "[s]")
            print(
                " launch clear velocity: ",
                "{0:.2f}".format(self.launch_clear_airspeed),
                "[m/s] at t=",
                "{0:.2f}".format(self.launch_clear_time),
                "[s]",
            )
            print("----------------------------")
            ...
            print(
                " Angle of attack for gust rate 2: ",
                "{0:.1f}".format(np.arctan(wind_speed / speed[Q_max]) * 180.0 / np.pi),
                "[deg]",
            )
            print("----------------------------")
            # print(time_2)
...
class PostProcess_dist:
    ...
    def set_coordinate_noshiro_riku(self):
        ...
        point_range = None
        point_rail = None
        with open("data/location_parameters/noshiro_sea.json", "r") as f:
        ...
        if point_range is None:
            raise ValueError("`range` parameter needed for `noshiro_sea.json`")
        if point_rail is None:
            raise ValueError("`rail` parameter needed for `noshiro_sea.json`")
        ...
        mat_rot = np.array([[np.cos(mag_dec), -1 * np.sin(mag_dec)], [np.sin(mag_dec), np.cos(mag_dec)]])
    ...
    def set_coordinate_izu(self):
        ...
        #  Load permitted range from json
        # -----------------------------------
        with open("data/location_parameters/izu.json", "r") as f:
            self.regulations = json.load(f)
        for reg in self.regulations:
            if reg["name"] == "rail":
    ...
    def set_coordinate_izu_sea(self):
        ...
        # -----------------------------------
        with open("data/location_parameters/izu_sea.json", "r") as f:
            self.regulations = json.load(f)
        for reg in self.regulations:
            if reg["name"] == "rail":
                self.point_rail = reg["center"]
        ...
        for i in range(self.xy_point[:, 0].size):
            ...
            line = sg.Line(
                sg.Point(self.xy_point[0, 0], self.xy_point[0, 1]), sg.Point(self.xy_point[1, 0], self.xy_point[1, 1])
            )
            ...
            self.b = (self.xy_point[0, 1] * self.xy_point[1, 0] - self.xy_point[1, 1] * self.xy_point[0, 0]) / (
    ...
    def set_coordinate_noshiro(self):
        ...
        #  Load permitted range here
        # -----------------------------------
        with open("data/location_parameters/fukuoka.json", "r") as f:
            self.regulations = json.load(f)
        for reg in self.regulations:
            if reg["name"] == "rail":
                self.point_rail = reg["center"]
    ...
    def plot_sct(self, drop_point, wind_speed_array, launcher_elev_angle, fall_type, savedir="./data/results/"):
        # -------------------
...
class JudgeInside:
    ...
    def judge_inside(self, check_point):
        ...
        if circle_flag2 == True:
            ...
            for center in range(center_num):
                # Compute distance between drop_point and center of limit circle
                length_point = np.sqrt(
                    (check_point[0] - self.outside_centers[center, 0]) ** 2
