...
def getEllipsePlot(a, b, Mrot=None, angle_rad=0, x0=0, y0=0, n_plots=100):
    ...
    if Mrot is None:
        Mrot = np.array([[np.cos(angle_rad), -np.sin(angle_rad)], [np.sin(angle_rad), np.cos(angle_rad)]])
...
def getAzimuthWind(center_pos, scale_vec, rotateMatrix, wind_direction_rad):
    unit_vec = np.array([np.sin(wind_direction_rad), np.cos(wind_direction_rad)])
...
def getProbEllipse(mu, sigma, alpha=0.95, n_plots=100):
    ...
    plots = getEllipsePlot(scale[0], scale[1], M, x0=mu[0], y0=mu[1], n_plots=n_plots)
    return plots[0], plots[1]


def getStatWindVector(statistics_parameters, wind_direction_deg, wind_std=None):
    ...
    for h in range(n_alt):
        ...
        #                     mu + wind_std[h]
        #                     )
        w = getAzimuthWind(mu + wind_std[h], scale, M, np.deg2rad(wind_direction_deg))

        stat_wind_u.append(w[0])
        stat_wind_v.append(w[1])
        print("Altitude: ", alt_axis[h], " ", stat_wind_u[h], ", ", stat_wind_v[h])
