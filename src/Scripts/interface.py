...
from Scripts.postprocess import JudgeInside, PostProcess_dist, PostProcess_single
...
class TrajecSimu_UI:
    ...
    def __init__(self, csv_filename, loc="izu_sea"):
        ...
        # provide csv file name that contains
    ...
    def run_loop(self, n_winddirec=16, max_windspeed=8.0, windspeed_step=1.0, output_rootdir="", overwrite_params=None):
        # =============================================
        ...
        mode = int(self.myrocket.Params.params_dict["azimuth_mode"])
        azimuth = int(self.myrocket.Params.params_dict["azimuth"])
        wind_direction_array = np.linspace(0 + (mode) * (azimuth), 360 + (mode) * (azimuth), n_winddirec + 1)
        wind_speed_array = np.arange(1.0, max_windspeed + windspeed_step, windspeed_step)
        ...
        for wind_speed in wind_speed_array:
            ...
            for wind_angle in wind_direction_array[:-1]:
                ...
                params_update[2][1] = 1.0e7
                params_update[3][1] = 1.0e7
        ...
        if self.myrocket.Params.params_dict["wind_model"] == "power-es-hybrid":
            ...
            output_dir = os.path.join(
                "./results/", "power-es-hybrid/", output_rootdir, str(wind_direction_original) + "deg/"
            )
        else:
            output_dir = os.path.join(
                "./results/", self.myrocket.Params.params_dict["wind_model"] + "/", output_rootdir
            )
        ...
        # -------------------------------
        output_kml(
            self.loc_bal, post_dist.point_rail, wind_speed_array, post_dist.regulations, output_dir + "plot_bal.kml"
        )
        output_kml(
            self.loc_para, post_dist.point_rail, wind_speed_array, post_dist.regulations, output_dir + "plot_para.kml"
        )
        ...
        # initialize dict
        permitted_area_for_para = {
            "range": None,
            "outside_centers": None,
            "outside_radius": None,
            "inside_center": None,
            "inside_radius": None,
            "under_line": None,
        ...
        if self.launch_location == "noshiro_sea":
            ...
            raise NotImplementedError(
                'The launch site is not implemented. "Noshiro_sea" or "izu" or "izu_sea" are currently available'
            )
        # END IF
    ...
    def output_loop_result(self, wind_speed_array, wind_direction_array, savedir="./data/results/"):
        ...
        judge_both.to_excel(excel_writer=excel_file, sheet_name="Go-NoGo判定 ")
        max_alt.to_excel(excel_writer=excel_file, sheet_name="最高高度 ")
        max_vel.to_excel(excel_writer=excel_file, sheet_name="最高速度 ")
        max_Mach.to_excel(excel_writer=excel_file, sheet_name="最大マッハ数")
        v_launch_clear.to_excel(excel_writer=excel_file, sheet_name="ランチクリア速度 ")
        v_para_deploy.to_excel(excel_writer=excel_file, sheet_name="パラ展開時速度 ")
        max_Q.to_excel(excel_writer=excel_file, sheet_name="最大動圧 ")
        loc_x_bal.to_excel(excel_writer=excel_file, sheet_name="弾道 x ")
        loc_y_bal.to_excel(excel_writer=excel_file, sheet_name="弾道 y ")
        loc_x_para.to_excel(excel_writer=excel_file, sheet_name="パラ x ")
        loc_y_para.to_excel(excel_writer=excel_file, sheet_name="パラ y ")
        bal_judge.to_excel(excel_writer=excel_file, sheet_name="弾道判定 ")
        para_judge.to_excel(excel_writer=excel_file, sheet_name="パラ判定 ")
        # Max acceleration
        max_accel.to_excel(excel_writer=excel_file, sheet_name="最大加速度 ")
