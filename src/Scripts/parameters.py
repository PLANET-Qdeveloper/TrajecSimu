...
class Parameters:
    ...
    def __init__(self, csv_filename):
        ...
        try:
            self.params_df = pd.read_csv(
                csv_filename, comment="$", names=("parameter", "value")
            )  # '$' denotes commet out
        except:
            self.params_df = pd.read_csv(
                csv_filename, comment="$", names=("parameter", "value", " ")
    ...
    def get_default(self):
        ...
        self.params_dict = {
            ...
            "elev_angle": 89.0,  # angle of elevation [deg]
            "azimuth": 0.0,  # north=0, east=90, south=180, west=270 [deg]
            "rail_length": 5.0,  # length of launcher rail
            # launch point configuration
            "latitude": 35.0,  # latitude of launch point [deg]
            # atmosphere property
            "T0": 298.0,  # temperature at 10m altitude [K]
            "p0": 1.013e5,  # pressure  at 10m alt. [Pa]
            # wind property
            "wind_direction_original": -1.0,
            "wind_direction": 315.0,  # azimuth where wind is blowing FROM
            "wind_speed": 2.0,  # wind speed at 'wind_alt_std' alt. [m/s]
            "wind_power_coeff": 14.0,
            "wind_alt_std": 5.0,  # alt. at which the wind speed is given [m]
            # wind model
            ...
            "Cmp": -0.0,  # stability derivative of rolling moment coefficient (aerodynamic damping)
            "Cmq": -4.0,  # stability derivative of pitching/yawing moment coefficient (aerodynamic damping)
            "Cl_alpha": 12.0,  # lift coefficient slope for small AoA [1/rad]
            ...
            "thrust_mag_factor": 1.0,  # thrust magnification factor
            "time_mag_factor": 1.0,  # burn time magnification factor
            # -----------------------------
            # parachute parameters
            # -----------------------------
            "t_deploy": 1000.0,  # parachute deployment time from ignition
            "t_para_delay": 1000.0,  # 1st parachute deployment time from apogee detection
            "Cd_para": 1.0,  # drag coefficient of 1st parachute
    ...
    def overwrite_parameters(self, params_df_userdef={}):
        ...
        try:
            ...
            self.rail_height = rail_length * np.sin(
                np.deg2rad(self.elev_angle)
            )  # height of launch rail in fixed coord.
            # launch point property
            self.omega_earth = np.array(
                [0.0, 0.0, -7.29e-5 * np.sin(np.deg2rad(float(self.params_dict["latitude"])))]
            ...
            if wind_direction_original != -1.0:
                print("Wind direction original Found")

            if (self.params_dict["wind_model"] == "power-es-hybrid") and wind_direction_original != -1.0:
            ...
            self.wind_unitvec = -np.array([np.cos(angle_wind), np.sin(angle_wind), 0.0])  # wind unitvector (blowing TO)

            self.wind_speed = float(self.params_dict["wind_speed"])  # speed of wind [m/s] at 10m alt.
            self.Cwind = 1.0 / float(self.params_dict["wind_power_coeff"])  # wind power coefficient
            ...
            # earth gravity
            self.grav = np.array([0.0, 0.0, -9.81])  # in fixed coordinate
        ...
        try:
            ...
            self.m_prop = float(
                self.params_dict["m_prop"]
            )  # propellant weight // NOTE: total weight at lift off = m_dry + m_prop
        ...
        try:
            ...
            self.X_area = np.pi * rocket_diameter**2.0 / 4.0  # cross-sectional area
            ...
            self.Mach_AOA_dependent = self.params_dict[
                "Mach_AOA_dep"
            ]  # flag whether aerodynamic parameters are dependent to Mach/AoA
            if type(self.Mach_AOA_dependent) == str:
                self.Mach_AOA_dependent = strtobool(self.Mach_AOA_dependent)
            self.aero_fin_mode = self.params_dict[
                "aero_fin_mode"
            ].strip()  # 'indiv' for individual fin computation, 'integ' for compute fin-body at once
            Cm_omega = np.array(
            ...
            self.Cm_omega_bar = (
                Cm_omega * np.array([rocket_diameter, rocket_height, rocket_height]) ** 2.0 * self.X_area
            )  # multiply with length^2. no longer non-dimansional
            ...
            if self.aero_fin_mode == "indiv":
                ...
                fin_y = np.linspace(fin_h / fin_dis / 2.0, fin_h - fin_h / fin_dis / 2.0, fin_dis)
                fin_xLE = (
                    1 / np.tan((90.0 - float(self.params_dict["fin_ang"]) * np.pi / 180.0)) * fin_y
                )  # leading edge locations
                self.fin_len = (
                    float(self.params_dict["fin_Ltip"]) * fin_y + (fin_h - fin_y) * float(self.params_dict["fin_Lroot"])
                ) / fin_h  # length of fin
                self.xFP = (
                    fin_xLE + self.fin_len / 4.0
                )  # forcing points  # array to store forcing-point locations (0=leading edge*root)
                self.r_arm = (
                    fin_y + float(self.params_dict["rocket_diameter"]) / 2.0
                )  # array of arm length for roll-spin
        ...
        try:
            ...
            self.t_para_delay = float(
                self.params_dict["t_para_delay"]
            )  # parachute deployment time from apogee detection
            ...
            if self.flag_2ndpara:
                ...
                self.t_deploy_2 = float(
                    self.params_dict["t_deploy_2"]
                )  # 2nd parachute deployment time from ignition# self.t_para_delay = float( self.params_dict['t_para_delay'] )   # 2nd parachute deployment time from apogee detection
    ...
    def setup_thrust(self, thrust_factor=1.0, time_factor=1.0):
        ...
        if self.thrust_input_type == "rectangle":
            ...
            if self.thrust_input_type == "curve_const_t":
                ...
                self.thrust_array = (
                    self.thrust_array[self.thrust_array >= 0.01 * np.max(self.thrust_array)] * thrust_factor
                )
            else:
        ...
        if self.thrust_input_type == "rectangle":
            ...
            if self.curve_fitting:
                ...
                thrust_poly[thrust_poly < 0.0] = 0.0  # overwrite negative value with 0
    ...
    def setup_aero_coeffs(self):
        ...
        if self.Mach_AOA_dependent:
            ...
            try:
                data = np.loadtxt("data/bin/Cd0.dat", delimiter=",", skiprows=1)
            except:
            ...
            try:
                data = np.loadtxt("data/bin/Clalpha.dat", delimiter=",", skiprows=1)
            except:
            ...
            try:
                df = pd.read_csv("data/bin/CPloc.csv", header=None, na_values="Mach/AOA")
            except:
                raise FileNotFoundError(" bin/CPloc.csv not found")
            Mach_array = np.array(df.iloc[1:, 0])  # mach array
            AOA_array = np.array(df.iloc[0, 1:]) * np.pi / 180.0  # AOA array (convert from deg to rad)
            # create grid
            Mach2, AOA2 = np.meshgrid(Mach_array, AOA_array)

            # CPlocation 2D array (rows: Mach, columns: AOA)
            CPloc_array = np.array(df.iloc[1:, 1:])
            # convert from % to len, and scaling. Use CPloc at M=0.3, AOA=0deg as standard
    ...
    def setup_wind(self):
        ...
        if self.wind_model == "power":
        ...
        elif self.wind_model == "power_Gust":
            self.wind = self.wind_power_Gust
            ...
            def wind_power_forecast(h):
                ...
                # NOTE: 2022/03/11: changed parameters for Izu-Riku Nov 2022
                boundary_alt = 300.0
                transition = 100.0
                ...
                if h <= boundary_alt - transition:
                    ...
                    # use both
                    weight = (h - (boundary_alt - transition)) / (2 * transition)
                    return weight * self.wind_forecast(h) + (1.0 - weight) * self.wind_power(h)
                else:
                    # use forecast only
            ...
            def wind_log_forecast(h):
                ...
                if h <= boundary_alt - transition:
                    ...
                    # use both
                    weight = (h - (boundary_alt - transition)) / (2 * transition)
                    return weight * self.wind_forecast(h) + (1.0 - weight) * self.wind_log(h)
                else:
                    # use forecast only
                    return self.wind_forecast(h)
            ...
            def wind_power_statistics(h):
                ...
                if h <= boundary_alt - transition:
                    ...
                    # use both
                    weight = (h - (boundary_alt - transition)) / (2 * transition)
                    return weight * self.wind_statistics(h) + (1.0 - weight) * self.wind_power(h)
                else:
                    # use statistics only
    ...
    # definition of wind power law for Gust
    def wind_power_Gust(self, h):
        ...
        # elif boundary_alt <= h :
        elif boundary_alt <= h < boundary_alt + 10:
            # wind velocity in local fixed coordinate (wind vector = direction blowing TO. Wind from west = [1,0,0])
            wind_vec = self.wind_unitvec * self.wind_speed * (h / self.wind_alt_std) ** self.Cwind + [5, 3, 0]
            return wind_vec

        elif boundary_alt + 10 <= h < boundary_alt2:
            # elif boundary_alt + 10 <= h < boundary_alt2:
            # wind velocity in local fixed coordinate (wind vector = direction blowing TO. Wind from west = [1,0,0])
            wind_vec = self.wind_unitvec * self.wind_speed * (h / self.wind_alt_std) ** self.Cwind + [5, -3, 0]
            return wind_vec

        elif boundary_alt2 <= h < boundary_alt2 + 5:
            # wind velocity in local fixed coordinate (wind vector = direction blowing TO. Wind from west = [1,0,0])
            wind_vec = self.wind_unitvec * self.wind_speed * (h / self.wind_alt_std) ** self.Cwind + [-7, -10, 0]
            return wind_vec

        elif boundary_alt2 + 5 <= h:
            # wind velocity in local fixed coordinate (wind vector = direction blowing TO. Wind from west = [1,0,0])
            wind_vec = self.wind_unitvec * self.wind_speed * (h / self.wind_alt_std) ** self.Cwind + [-7, -10, 0]
            return wind_vec
