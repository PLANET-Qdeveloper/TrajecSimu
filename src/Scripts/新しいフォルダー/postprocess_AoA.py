...
class PostProcess_single:
    ...
    def get_flight_detail(self, time, show=False):
        ...
        # ikeda write
        # nagazawa rewrite in 2022/02/17 only for AoA
        M = speed
        # Q = 0.5 * rho * speed**2.
        # AoA = np.arctan( wind_speed/speed)*180./np.pi
        # u = self.myrocket.trajectory.solution[:,3]
        Q = np.linalg.norm(self.myrocket.trajectory.solution[:, 3:4], axis=1)
        AoA = np.linalg.norm(self.myrocket.trajectory.solution[:, 4:5], axis=1)
        u = np.linalg.norm(self.myrocket.trajectory.solution[:, 5:6], axis=1)
        U = pd.DataFrame(u)
        p = np.linalg.norm(self.myrocket.trajectory.solution[:, 2:3], axis=1)
        t = time
        ...
        if show:
            ...
            print(
                ...
                " at t=",
                "{0:.2f}".format(time[int(self.myrocket.trajectory.max_accel)]),
                "[s]",
            )
            print(
                " Max. altitude: ",
