...
class PostProcess_single:
    ...
    def get_flight_detail(self, time, show=False):
        ...
        df_6 = pd.DataFrame({"time": time})

        numpy.set_printoptions(threshold=numpy.inf)
        df.to_csv("./data/results/AoA.csv", sep="\t")
        df_1.to_csv("./data/results/Q.csv", sep="\t")
        df_2.to_csv("./data/results/M.csv", sep="\t")
        df_3.to_csv("./data/results/T.csv", sep="\t")
        df_4.to_csv("./data/results/P.csv", sep="\t")
        df_5.to_csv("./data/results/u.csv", sep="\t")
        df_6.to_csv("./data/results/time.csv", sep="\t")
    ...
    def plot_acceleration(self, time):
        # =============================================
        # this method plots acceleration as a function of time by Wada
        # =============================================

        # ug on the rail -> du/dx only. Consider rail-rocket friction force
        # total acceleration
        u = self.myrocket.trajectory.solution[:]
        x, v, q, omega = self.state2vecs_quat(u)
        Tbl = quaternion.as_rotation_matrix(np.conj(q))
        dv_dt = -np.cross(omega, v) + np.dot(Tbl, self.Params.grav) + (aeroF + self.thrust1(t) + self.friction1()) / mass
        # cancell out y,z
        dv_dt = np.array([dv_dt[0], 0., 0.])
        dx_dt = np.dot(Tbl.T, v)

        Accel = np.linalg.norm(dv_dt)

        plt.figure(8)
        # acceleration history
        plt.plot(time, Accel, lw=2, label='Acceleration')
        plt.legend()
...
class PostProcess_dist:
    ...
    def set_coordinate_noshiro(self):
        ...
        # -----------------------------------
        # used as "outside_centers" & "outside_radius", meaning NOT drop inside the circle
        point_rail = np.array([40.241735, 140.010157])
        point_switch = np.array([40.240382, 140.009295])
        point_tent = np.array([40.2414098, 140.0100285])
        self.lim_radius = 50.0

        # used as "inside_center" & "inside_radius", meaning MUST drop inside the circle
        self.hachiya_radius = 2000.0  # [m]
        point_center = np.array([40.246803, 139.987536])  # center of circle

        # used as two points of "over_line", meaning MUST drop over the line
        point_point = np.array(
            [
                [40.243015, 140.007566],
                [40.235585, 140.005619],
            ]
        )
