...
class Trajec_main:
    ...
    def ODE_main(self):
        ...
        if self.Params.integ == "lsoda_odeint":
            ...
            # wada fixed (if u wanna dt for initial 3 sec is 10 times smaller,change"np.arange(t0,3.,self.Params.dt/10)")
    ...
    def f_main(self, u, t):
        ...
        # elif self.flag==3.5 and (t >= self.t_deploy_2 or 高度が規定値以下) :
            ...
            point_rail = [self.Params.loc_lat, self.Params.loc_lon]  # 入力: 射点の座標を入れよ
            mag_dec = abs(self.Params.loc_mag)  # [deg]
            mag_dec1 = (mag_dec * np.pi) / 180
            lat2met = 2 * np.pi * earth_radius / 360.0
            lon2met = 2 * np.pi * earth_radius * np.cos(np.deg2rad(point_rail[0])) / 360.0

            lat = (
                (np.round(x[1], 4) * np.cos(mag_dec1) + np.round(x[0], 4) * np.sin(mag_dec1)) / lat2met
            ) + point_rail[0]

            lon = (
                (np.round(x[0], 4) * np.cos(mag_dec1) - np.round(x[1], 4) * np.sin(mag_dec1)) / lon2met
            ) + point_rail[1]

            # 経度が有効な範囲（-180度から180度）内にあるように調整
            lon = (lon + 180) % 360 - 180

            point = np.array([np.round(x[0], 3), np.round(x[1], 3)])
            ...
            print("                 latitude =", (np.round(lat, 5)))
            print("                longitude =", (np.round(lon, 5)))
            print("----------------------------")
            print(" ")
            # record landing time
            self.landing_time = t
    ...
    def mass_MOI(self, t):
        ...
        if t >= self.Params.t_MECO:
            ...
            Impulse_so_far = integrate.trapezoid(self.Params.thrust_function(time_so_far), time_so_far)
            ...
            # ---------------------------------
            h = 1.0e-3
            Impulse_so_far = integrate.trapezoid(
