...
# 必須の引数
parser.add_argument("src_filename", help="入力の予報風csvファイル名。")
parser.add_argument("search_foldername", help="検索候補となる過去の予報風csvファイルが入っているフォルダ名。")
...
src_wind = np.c_[src_u_func(alt_axis), src_v_func(alt_axis), src_w_func(alt_axis)]
...
for search_filename in search_filenames:
    ...
    wind = np.c_[u_func(alt_axis), v_func(alt_axis), w_func(alt_axis)]
